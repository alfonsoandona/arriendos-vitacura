# Acreditar 3, Vitacura

**84/100** · `#AAHDV` · Vitacura · a 3,34 km del Sport Francés · [📍 abrir en Google Maps](https://www.google.com/maps/search/?api=1&query=Acreditar+3%2C+Vitacura%2C+Vitacura%2C+Chile)

## Cuánto cuesta

| | |
|---|---|
| Arriendo | $1.552.602 |
| Gastos comunes | $430.000 (27,7% del canon) |
| **Costo mensual** | **$1.982.602** |
| Publicado en UF | UF 38 |
| Por m² | $11.090 / m² |

## Qué es

| | |
|---|---|
| Tipo | departamento |
| Superficie total | 140 m² |
| Superficie útil | 140 m² |
| Dormitorios | 4 |
| Baños | 3 |
| Estacionamientos | 2 |
| Bodega | sí |
| Antigüedad | — |
| Piso | 6 |
| Orientación | oriente |
| Disponible | ya |

## De dónde sale el puntaje

| Rubro | Puntos | Qué se midió |
|---|---|---|
| Ubicación | 19/26 | Vitacura · a 3,3 km — fuera del anillo |
| Antigüedad | — /24 | el aviso no publica el año · _falta el año de construcción_ |
| Precio | 13/20 | $1.552.602 · GC $430.000 · 27.7% del canon — altos |
| Superficie | 14/16 | 140 m² totales |
| Programa | 13/14 | 4D · 3B |
| _Preferencias_ | +6 | desempate entre las que ya calificaron |

> El puntaje se midió sobre **76 de 100** puntos posibles: hay rubros que este aviso no publica. Con esos datos podría llegar a **89/100**.
>
> Por eso un puntaje bajo acá no significa necesariamente un mal departamento: puede ser uno del que se sabe poco.

<details><summary>Detalle criterio por criterio</summary>

- Ubicación: Vitacura · a 3,3 km — fuera del anillo
- Precio: $1.552.602 · GC $430.000 · 27.7% del canon — altos
- Superficie: 140 m² totales
- Programa: 4D · 3B
- piso 6
- orientación oriente
- 35 m² por dormitorio
- 2 estacionamientos
- con bodega
- logia

</details>

## Qué preguntar antes de ir

- [ ] **Los gastos comunes son altos** (27,7% del canon). Preguntar qué incluyen y si hay algún gasto extraordinario vigente — una reparación de fachada se reparte entre todos los departamentos y puede durar años.
- [ ] **¿De qué año es el edificio?** Define casi un cuarto del puntaje de este radar y el aviso no lo publica.
- [ ] **¿Cuántos meses de garantía?** Con dos meses sobre este canon son $3.105.204 al firmar, además del primer mes y la comisión.
- [ ] **¿Cuánto es la comisión de corretaje?** Lo habitual es medio mes más IVA, a cargo del arrendatario.
- [ ] **¿El reglamento del edificio acepta mascotas?**
- [ ] **¿Está disponible para visitar esta semana?** En Vitacura, un departamento de más de 100 m² con 3 dormitorios dentro de tu presupuesto se toma en días.

## Dónde está publicado

- [TocToc](https://www.toctoc.com/propiedades/arriendocorredorasr/departamento/vitacura/2-estacionamientos-140m2-vitacura/0a473ba32517432c7fe82ad504ac1495b8678e88)

## ✏️ ¿Lo viste o llamaste? Anótalo

En [`gestion.yml`](../../gestion.yml), desde el teléfono. El radar lo recuerda para siempre: un `descartado` no vuelve a sonar, y lo que averigües entra al puntaje.

```yaml
  - codigo: AAHDV
    estado: visita        # descartado | visto | contactado | visita
    # ano_construccion: 2015
    # nota: "lo que te dijeron"
```

---

<details><summary>Datos crudos del aviso</summary>

```
2 estacionamientos 140m2 Vitacura Departamento en arriendo, con entrega inmediata, a pasos de Nueva Costanera y Vespucio Norte, comuna de Vitacura.

-140m2.
- Piso 6.
- Orientación Sur- Oriente. 
- Cocina tradicional cerrada con cómoda Logia ventilación natural.
- Living comedor con acceso a terraza.
- 4 Dormitorios tradicionales más un 4to chico.
- 3 Baños. 
- 2 Estacionamientos. 
- 1 Bodega. 
- Gasto comun: $430.000 aprox. 

Requisitos de arriendo :
- Acreditar 3 veces el valor de arriendo con:
3 últimas liquidaciones de sueldo. (Se puede complementar)
- contrato de trabajo o certificado de antigüedad laboral.
- cotizaciones de afp.
- carnet vigente ambos lados.
- no tener protestos en DICOM presentando el Platinum 360 de equifax. 
- Aval con mismos requisitos o mes extra de garantía. 

No se acepta pagos de año completo , no se acepta contratos de arriendo para acreditar renta.

En caso de independientes acreditar renta con carpeta triburaria para creditos (no aceptan boletas o resumen de boletas)

Considerar :
Mes de arriendo. 
Mes de garantía. 
50% &amp;#43; iva de corretaje 

Arrienda Broker and Partner   Vitacura
```

Leído de `toctoc` vía `estado-embebido` el 19-08-2026 04:13 UTC.

</details>
